...
- Para preparar ambiente Android local sem drift, execute `./tools/ci/prepare_android_env.sh`; o script prioriza `ANDROID_SDK_ROOT`/`ANDROID_HOME`, faz fallback em `${REPO_ROOT}/.android-sdk`, `/workspace/android-sdk`, `/usr/lib/android-sdk`, `/opt/android-sdk`, `/opt/android-sdk-linux` e `$HOME/Android/Sdk`, e sincroniza `sdk.dir` + `ndk.dir` (quando `ndk.version` existir no SDK) em `local.properties`.
