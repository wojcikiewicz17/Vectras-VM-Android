#!/usr/bin/env python3
"""Validate CI workflow matrix/profile consistency and host workflow canonicalization."""
...
if ci_alias.exists():
    ...
    if './.github/workflows/host-ci.yml' not in ci_content:
        ...
        sys.exit(1)
    forbidden_alias_markers = [
        'validate_pipeline_directories.sh --profile host',
        'make run-selftest',
        'check_incoming_ingestion.py',
        'cmake --build build-cmake',
    ]
    for marker in forbidden_alias_markers:
        if marker in ci_content:
            print(f'ci.yml must stay alias-only; found host responsibility marker: {marker}')
            sys.exit(1)
...
host_semantic_workflows = [
    ...
    if p.name not in {'host-ci.yml', 'ci.yml'} and 'host' in p.stem.lower()
]
