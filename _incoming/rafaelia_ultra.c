...
void process_ultra(rafaelia_t *out, rafaelia_t *prev, rafaelia_t *in) {
    ...
    // Unroll de 2x: processa 8 floats por iteração (32 bytes)
...
int main() {
    ...
    for(int k=0; k<TORUS_SIZE; k++) p.v[k] = 1.0f; 
    process_ultra(&o, &p, &i);
    printf("--- RAFAELIA ULTRA CORE (ARM32) ---\n");
    printf("Tempo: %llu ns | Saída[0]: %.4f\n", o.ns, o.v[0]);
    return 0;
}
